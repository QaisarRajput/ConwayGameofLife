...
from update_display import CAWindow 
...
class ConwaysRule(Rule):
    ...
    def evolve_cell(self, last_cell_state, neighbors_last_states):
        ...
        if last_cell_state == DEAD and 2 <= alive_neighbours <= 4:
...
#        
        ...
        if last_cell_state == ALIVE and 1 < alive_neighbours < 4:
            new_cell_state = ALIVE
        if last_cell_state == ALIVE and 3 < alive_neighbours < 6:
            new_cell_state = DEAD
            
        if alive_neighbours >= 6:
            new_cell_state = DEAD
...
if __name__ == "__main__":
    neighborhood = MooreNeighborhood(EdgeRule.FIRST_AND_LAST_CELL_OF_DIMENSION_ARE_NEIGHBORS, radius=1)
#    neighborhood = HexagonalNeighborhood(EdgeRule.FIRST_AND_LAST_CELL_OF_DIMENSION_ARE_NEIGHBORS)
    
    ca = CAFactory.make_multi_process_cellular_automaton(dimension=[300, 300],
